...
type DebtorDetailRow = {
  ...
  route?:
    | { id?: string | null; name?: string | null; area?: string | null }
    | Array<{ id?: string | null; name?: string | null; area?: string | null }>
    | null;
  collector?:
    | {
        id?: string | null;
...
export type CollectorDebtorPayload = {
  debtor?: {
    id: string;
    name: string;
    phone: string | null;
...
export class MobileDebtorDetailError extends Error {
  status: number;
...
export async function loadMobileDebtorDetailByClerkId(userId: string, id: string): Promise<CollectorDebtorPayload> {
  const session = await getAppSessionContextByClerkId(userId);
  if (session.role !== "collector" && session.role !== "creditor") {
    throw new MobileDebtorDetailError("Access denied", 403);
  }
  ...
  let { data: debtor, error: debtorError }: { data: DebtorDetailRow | null; error: QueryError } =
    await debtorQuery.maybeSingle();

  if (
    debtorError &&
    (debtorError.message.includes("requested_by_collector_id") ||
      isMissingDebtorColumnError(debtorError.message, "portal_email") ||
      isMissingDebtorColumnError(debtorError.message, "approval_status") ||
      isMissingDebtorColumnError(debtorError.message, "approval_requested_at") ||
      isMissingDebtorColumnError(debtorError.message, "approval_note")) &&
  ...
  if (debtorError || !debtor) {
    throw new MobileDebtorDetailError(debtorError?.message || "Debtor not found", 404);
  }
  ...
  if (loanResult.error) {
    throw new MobileDebtorDetailError(loanResult.error.message, 500);
  }

  if (collectionResult.error) {
    throw new MobileDebtorDetailError(collectionResult.error.message, 500);
  }
  ...
  const activeLoanCount = loans.filter((loan) =>
    ["active", "approved", "pending_approval", "overdue"].includes(String(loan.status || ""))
  ).length;
  ...
  const kycVerified = Boolean(
    debtor.is_verified || (debtor.id_photo_front_url && debtor.debtor_photo_url && debtor.signature_url)
  );
